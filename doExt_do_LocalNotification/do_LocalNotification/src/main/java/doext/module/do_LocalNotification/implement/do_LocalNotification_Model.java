...
public class do_LocalNotification_Model extends DoSingletonModule implements do_LocalNotification_IMethod {
	...
	public void addNotify(JSONObject _dictParas, DoIScriptEngine _scriptEngine, DoInvokeResult _invokeResult) throws Exception {
		try {
			...
			if (TextUtils.isEmpty(_repeatMode) || _repeatMode.equals("None")) {// 为空并且格式为yyyy-MM-dd
																				// HH:mm:ss就执行一次
	...
	private void sendLocalNotification(boolean isVibrate, String ringing, Date notifyTimeDate, String title, String content, int notifyId, JSONObject extra, long time) {
		...
		if (time == -1) {
			...
			// notifyTimeDate.getTime(), sender);
			if (Build.VERSION.SDK_INT < 23) {
				if (Build.VERSION.SDK_INT >= 19){
					try {
						alarmManager.setExact(AlarmManager.RTC_WAKEUP, notifyTimeDate.getTime(), sender);
					} catch (Exception localException) {
						localException.fillInStackTrace();
						alarmManager.set(AlarmManager.RTC_WAKEUP, notifyTimeDate.getTime(), sender);
					}
				}else {
					alarmManager.set(AlarmManager.RTC_WAKEUP, notifyTimeDate.getTime(), sender);
				}
			}else{
				alarmManager.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, notifyTimeDate.getTime(), sender);
			}
		} else {
